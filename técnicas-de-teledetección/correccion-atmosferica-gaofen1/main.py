...
def _(Reflectancia, mo):
    pixel_x = mo.ui.slider(start=0, stop=Reflectancia.shape[2] - 1, step=1, value=0, label="pixel_x")
    pixel_y = mo.ui.slider(start=0, stop=Reflectancia.shape[1] - 1, step=1, value=0, label="pixel_y")
...
def _(Reflectancia, data1, mo, np, pixel_x, pixel_y, plt):
    ...
    _fig.colorbar(_plot, ax=_ax2, aspect=20).set_label("Reflectance", rotation=90, labelpad=20)
    _ax2.ticklabel_format(useOffset=False, style="plain")
    for label in _ax2.get_xticklabels():
        label.set_rotation(90)
    _ax2.plot(data1.bounds[0] + pixel_x.value, data1.bounds[3] - pixel_y.value, "s", markersize=5, color="red")
