...
# Metadata del raster
band_4 = raster_b4.GetRasterBand(
    1
)  # Convierte el dataset raster a Banda en una variable separada
print(type(band_4))  # Chequea en tipo de variable
osgeo.gdal.GetDataTypeName(band_4.DataType)  # Tipo de dato de los valores
if (
    band_4.GetMinimum() is None or band_4.GetMaximum() is None
):  # Calculo de estadisticas necesarias
...
array_4x = np.ma.masked_equal(
    array_4x, nodata
)  # Enamascara los valores NoData de la imagen
type(array_4x)
